 ...
 */
public class MultiJVMRunner {

	protected static final Logger logger = Logger.getLogger(MultiJVMRunner.class);
	...
	public MultiJVMRunner(List<String[]> programArgs) {
	 ...
	 */
	public MultiJVMRunner(String[]... programArgs) {
		this.setCommandArgs(programArgs);
	}

	public MultiJVMRunner(long waitTimeBeforeKill, String[]... programArgs) {
		this(programArgs);
		this.waitTimeBeforeKill = waitTimeBeforeKill;
	}

	public MultiJVMRunner(long waitTimeBeforeKill, HeapSpec[] heapSpecs, String... programArgs) {
		...
		Thread killThread = new Thread(new Runnable() {
			...
			public void run() {
				try {
					...
					logger.fine("Kill thread was interrupted");
				}
			}
	...
	private List<String> getCoreProgArgs(int procIndex, HeapSpec heapSpec) {
		...
		if (PlatformType.probe().equals(PlatformType.WINDOWS)) {
	...
	private class StreamReaderThread extends Thread {
		...
		StreamReaderThread(InputStream is, String name) {
			this(is, name, true);
		}
		StreamReaderThread(InputStream is, String name, boolean bufferData) {
			...
			if (!file.exists()) {
				try {
					...
					logger.warn(EntityStoreMessageCode.CHILD_PROCESS_STREAM_EXCEPTION, e, new Object[] { name });
				}
			}
			try {
		...
		void consumeStreamFast(InputStream is, String streamName, StringBuilder outputBuffer, boolean consumeFully) {
			...
			try {
				...
				logger.warn(EntityStoreMessageCode.CHILD_PROCESS_STREAM_EXCEPTION, ioe, new Object[] { streamName });
			} finally {
				try {
	...
	public static void main(String[] args) {
		try {
			...
			initClass(true);
			MultiJVMRunner runner = new MultiJVMRunner(progArgs);
			if (heapSpecs != null) {
	...
	public static String getUsage() {
		String className = MultiJVMRunner.class.getSimpleName();
